...
module flex_pts_sr_NUM_BITS4_SHIFT_MSB0 ( clk, n_rst, shift_enable, 
  ...
  wire   [3:1] output_data;

  DFFSR \output_data_reg[3]  ( .D(n17), .CLK(clk), .R(1'b1), .S(n_rst), .Q(
        output_data[3]) );
  DFFSR \output_data_reg[2]  ( .D(n18), .CLK(clk), .R(1'b1), .S(n_rst), .Q(
        output_data[2]) );
  DFFSR \output_data_reg[1]  ( .D(n19), .CLK(clk), .R(1'b1), .S(n_rst), .Q(
        output_data[1]) );
  DFFSR \output_data_reg[0]  ( .D(n16), .CLK(clk), .R(1'b1), .S(n_rst), .Q(
        serial_out) );
  OAI21X1 U7 ( .A(n5), .B(n6), .C(n7), .Y(n19) );
  AOI22X1 U8 ( .A(output_data[2]), .B(n8), .C(output_data[1]), .D(n9), .Y(n7)
         );
  INVX1 U9 ( .A(parallel_in[1]), .Y(n6) );
  OAI21X1 U10 ( .A(n5), .B(n10), .C(n11), .Y(n18) );
  AOI22X1 U11 ( .A(output_data[3]), .B(n8), .C(n9), .D(output_data[2]), .Y(n11) );
  INVX1 U12 ( .A(parallel_in[2]), .Y(n10) );
  NAND2X1 U13 ( .A(n12), .B(n13), .Y(n17) );
  INVX1 U14 ( .A(n8), .Y(n13) );
  MUX2X1 U15 ( .B(parallel_in[3]), .A(output_data[3]), .S(n9), .Y(n12) );
  OAI21X1 U16 ( .A(n5), .B(n14), .C(n15), .Y(n16) );
  AOI22X1 U17 ( .A(output_data[1]), .B(n8), .C(serial_out), .D(n9), .Y(n15) );
